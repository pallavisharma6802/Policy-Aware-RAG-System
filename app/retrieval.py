...
from app.embeddings import get_embedding_model, EMBEDDING_MODEL_NAME
...
class HybridRetriever:
    ...
    def vector_search(self, query: str, limit: int = 10) -> List[Dict]:
        ...
        return result.get("data", {}).get("Get", {}).get("PolicyChunk", [])
    ...
    def sql_filter(
        ...
        if region:
            query = query.filter(PolicyChunk.region == Region(region.strip().lower()))
        if content_type:
            query = query.filter(PolicyChunk.content_type == ContentType(content_type.strip().lower()))
        if policy_source:
            query = query.filter(PolicyChunk.policy_source == PolicySource(policy_source.strip().lower()))
        return query.all()
    ...
    def retrieve(
        ...
        vector_results = self.vector_search(query=query, limit=limit * 3)
